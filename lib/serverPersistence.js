...
function ServerPersistence(aCachedEntries, aConnectParameters, aLogLevel, aModules, aPrefix) {
  ...
  function getLockKey(aKey) {
    return getKey(aKey) + LOCK_SUFFIX;
  }
  ...
  return {
    ...
    // active wait with delays) since locking on a single thread app is not going to work that great.
    async getAndLockKey(aKeyName, aAsObject, aTimeout) {
      ...
      provider.psetex(lockKey, 1, aTimeout); // In case we don't call release...
      return this.getKey(aKeyName, aAsObject);
    },
    releaseLock(aKeyName) {
      return provider.del(getLockKey(aKeyName));
    }
  };
