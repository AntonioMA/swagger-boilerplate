!function(global) {
  ...
  // Note that this implementation *requires* WeakMap, so it won't work on IE9.
  ...
  Utils.CachifiedObject = function(aCachedObject) {
    ...
    if (getNewInstance) {
      cachedObject.instance =
       new (Function.prototype.bind.apply(aCachedObject, arguments)); /*jshint ignore: line*/
  ...
  Utils.isA = function _isA(aTemplate, aObject, aAllowEmptyArrays) {
    ...
    if (templateIsArray && objectIsArray) {
      return (aObject.length !== 0 && aObject.every &&
              aObject.every(function(element) {
                return _isA(aTemplate[0], element, aAllowEmptyArrays);}
              )) ||
              aObject.length === 0 && aAllowEmptyArrays;
    }
    if (templateIsArray || objectIsArray) {
      return false;
    }
    ...
    return [].every && Object.keys(aTemplate).every(function(aKey) {
      return typeof aObject[aKey] === typeof aTemplate[aKey] &&
        (typeof aObject[aKey] !== 'object' ||
         _isA(aTemplate[aKey], aObject[aKey], aAllowEmptyArrays));
    });
  };
  ...
  Utils.sanitizeId = id => (typeof id === 'string' && id.toLowerCase().trim()) || id;
