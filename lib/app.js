...
module.exports = function App(aOptions, aModules) {
  ...
  const MUST_PARSE_ORIGIN = Array.isArray(aOptions.allowedCorsOrigins)
  const getOriginHeader = allowedCORSOrigins =
   Array.isArray(aOptions.allowedCorsOrigins) && aOptions.allowedCorsOrigins;
  ...
  var corsTemplate = function(aAllowedHeaders, aMethod, aReq, aRes, aNext) {
  ...
  var enableCORS = ['put', 'get', 'post', 'delete', 'patch'].reduce((aPrevious, aVerb) => {
