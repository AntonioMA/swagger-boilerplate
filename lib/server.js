...
module.exports = function(aOptions) {
  ...
  try {
    ...
    var staticOptions = options.staticOptions || {};
    var serverPort = options.serverPort;
    var allowedCORSOrigins = options.allowedCORSOrigins.split(',');
  ...
  function start() {
    ...
    Promise.all([loadServerConfig, loadYAML(aApiFile)]).then(requisites => {
