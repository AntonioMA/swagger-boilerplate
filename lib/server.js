...
module.exports = function(aOptions) {
  ...
  function start() {
    ...
    return Promise.all([loadServerConfig, loadYAML(aApiFile), processSetup]).then((requisites) => {
      ...
      const serverPortRaw = String.raw`${options.serverPort}`;
      const pidStreamId = serverPortRaw.replace("\\\\.\\pipe\\", '');
